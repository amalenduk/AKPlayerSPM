...
public func makeAKMedia(from test: TestMedia) -> AKMedia? {
    ...
    let headers = ["User-Agent": "AKPlayerExample/1.0"]
    let staticMetadata = AKNowPlayableStaticMetadata(assetURL: url, mediaType: test.kind.akMediaType == .clip ? .video : .audio, isLiveStream: test.kind == .live, title: test.name, artist: nil, artwork: .none, albumArtist: nil, albumTitle: nil)
